...
object MemoryRepository : HotelRepositorio {// será uma instancia unica
    private var nextId = 1L// será incrementado toda vez uque um nobo objt for inserido
    private val hotelsList = mutableListOf<Hotel>()// armazenando a lista
    ...
    override fun save(hotel: Hotel) {// verificando
        if (hotel.id == 0L) {
            hotel.id = nextId++
            hotelsList.add(hotel)
        } else {// fazendo busca pelo id
    ...
    override fun search(term: String, callback: (List<Hotel>) -> Unit) {// retornara uma nova lista
        // com os intens filtrados pelo nome.
        callback(
